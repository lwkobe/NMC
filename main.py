...
from loss_float_gradient_code import get_loss_float_gradient
...
parser.add_argument("--data_dir", action="store", dest="data_dir", default="./groundtruth/gt_NMC", help="Root directory of dataset [gt_NMC,gt_simplified,?]")
...
if is_training:
    ...
    dataset_train = dataset.ABC_nmc_hdf5(FLAGS.data_dir, train=True, input_type=FLAGS.input_type, out_bool=dataset_bool, out_float=dataset_float)
    dataset_test = dataset.ABC_nmc_hdf5(FLAGS.data_dir, train=False, input_type=FLAGS.input_type, out_bool=True, out_float=True)
        ...
        for i, data in enumerate(dataloader_train, 0):
            ...
            if dataset_float:
                ...
                if FLAGS.input_type == "sdf":
                    ...
                    #MSE
                    loss_float_value = torch.sum(( (pred_output_float-gt_output_float)**2 )*gt_output_float_mask)

                    #This part is generated automatically via generate_loss_float_gradient_code.py
                    #make gradients equal (gt_0 - gt_1 = pred_0 - pred_1)
                    loss_float_gradient = get_loss_float_gradient(pred_output_float,gt_output_float,gt_output_float_mask)

                    loss_float = (loss_float_value*0.1 + loss_float_gradient)/torch.clamp(torch.sum(gt_output_float_mask),min=1)
    ...
    if dataset_bool and dataset_float: #only read input
        dataset_test = dataset.ABC_nmc_hdf5(FLAGS.data_dir, train=False, input_type=FLAGS.input_type, out_bool=True, out_float=True, input_only=True)
        dataloader_test = torch.utils.data.DataLoader(dataset_test, batch_size=1, shuffle=False, num_workers=16)  #batch_size must be 1
    else:
        dataset_test = dataset.ABC_nmc_hdf5(FLAGS.data_dir, train=False, input_type=FLAGS.input_type, out_bool=True, out_float=True)
        dataloader_test = torch.utils.data.DataLoader(dataset_test, batch_size=1, shuffle=False, num_workers=16)  #batch_size must be 1
